...
/*
Movement is done within the KeyInput.java file
*/
...
public class Player extends GameObject {
    ...
    public void tick() {
        ...
        for (int i = 0; i < objHandler.list.size(); i++) {
            ...
            if (tempObj.getID() == ID.Enemy || tempObj.getID() == ID.Bullet) {
                if (bc.intersects(tempObj.bc)) { // If the player touches an enemy/bullet
                    if (onlyOnce) {
                        Health -= 2;
                        onlyOnce = false;
                    }
