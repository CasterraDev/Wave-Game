    ...
    public void tick() {
        ...
        if (Game.state == Game.STATE.game) {
            if (timer > 50 && curLvl == 1) { // Lvl 2
                objHandler.addObject(new Turret(1, 1, 24, 24, ID.Turret, objHandler));
                ...
                objHandler.addObject(new SpeedEnemy(r.nextInt(Game.WIDTH - 64), r.nextInt(Game.HEIGHT - 64), ID.Enemy));
                objHandler.addObject(new FollowerEnemy(r.nextInt(Game.WIDTH - 64), r.nextInt(Game.HEIGHT - 64), 64, 64,
                        ID.Enemy, objHandler));
                hud.setLevel(curLvl + 1);
                ...
                objHandler.addObject(new SpeedEnemy(r.nextInt(Game.WIDTH - 64), r.nextInt(Game.HEIGHT - 64), ID.Enemy));
                hud.setLevel(curLvl + 1);
            } else if (timer > 450 && curLvl == 10) { // YOU WIN!!!!!
                Game.state = Game.STATE.winScreen;
            }
        }

        // Basic way to add an enemy
        // objHandler.addObject(new BasicEnemy(r.nextInt(Game.WIDTH - 64),
        // r.nextInt(Game.HEIGHT - 64),64,64, ID.Enemy,objHandler));
    }
}
